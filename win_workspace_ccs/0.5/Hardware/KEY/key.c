 ...
 *
 *   上电 (main() 启动序列里调一次):
 *     KEY_Init();                        // K5 (PB21) 开内部上拉, 清状态机
 *
 *   SysTick_Handler 每 1ms 自动调 (clock.c 已挂, 应用层无需再调):
 *     KEY_Tick();                        // 推进 5 路按键内部状态机
 *
 *   主循环消费事件 (一次性, 读后自动清):
 *     if (key(1, down))        { ... }   // K1 下降沿 (松开 → 按下)
 *     if (key(2, up))          { ... }   // K2 上升沿 (按下 → 松开)
 *     if (key(3, long_press))  { ... }   // K3 长按 (按住 ≥ KEY_LONG_MS=1500)
 *
 *   LCD 显示当前物理电平 (不消费):
 *     if (key_pressed(5))      { ... }   // K5 当前是否按下 (true=按下)
 *
 * ============================================================================
 * 内部状态机 (每个键独立):
 *    S0_IDLE      松开, 无事件
 ...
 *
 * 短按一下的完整路径:
 *   S0_IDLE -> (下降沿) -> S1_PRESSED -> 发 down -> (松开) -> S3_UP -> 发 up -> S0_IDLE
 *
 * 长按一下的完整路径:
 *   S0_IDLE -> (下降沿) -> S1_PRESSED -> 发 down
 *   -> (按住 KEY_LONG_MS=1500) -> 发 long -> S2_LONGED -> (松开) -> S3_UP -> 发 up -> S0_IDLE
 *
 * 一次调用 key(id, type) 消费一个事件 (读后自动清零), 重复调用读到 false.
 * ============================================================================
 */
