...
typedef struct {
    ...
    /* 读位置自动重试 (K3 短按 / 主循环定时触发读当前位置)
     *   - 2026-07-18 用户反馈: "主页 X 轴脉冲值不更新" → 同一 tick_ms 内 X/Y
     *     同时 SM_ReadPosition, 旧字段共享导致后调用的轴覆盖先调用的轴,
     *     只读到了 Y 轴. 改为每轴独立缓存, 节流器先发 X 再发 Y (顺序)
     *   - read_addr[axis]: 调用方写入的"目标从机地址" (SM_X=0x01 / SM_Y=0x02)
     *   - read_active_addr: SM_Tick 选定要发的那一帧的从机地址, sm_emit 用 */
    bool             read_pending[2];
...
void SM_Tick(void) {
    ...
    /* 2. K3 / 主循环定时读位置自动重试: 收到应答 (SM_AckFrame) 或 500ms 超时停
     *    2026-07-18 修复: 改为每轴独立 pending/last_ms/start_ms, 两轴同时请求
     *    都能记住, 节流器按"先 X 后 Y"顺序逐帧发出。 */
    if (s_thr.pending == SM_OP_NONE) {
        for (uint8_t axis = 0U; axis < 2U; axis++) {
            ...
            if ((int32_t)(now - s_thr.read_last_ms[axis]) >= (int32_t)SM_FRAME_GAP_MS) {
                /* 挑中这一轴: 写入 read_active_addr (SM_X=0x01 / SM_Y=0x02),
                 *   由 sm_emit 转发到 PD42S1_ReadPosition
                 *   read_addr[axis] 是 SM_ReadPosition 入口写入的目标从机地址 */
...
void SM_Init(void) {
    /* 用户 2026-07-16 反馈 cold-boot hang: Y 轴驱动器在 0.4 起真驱动,
     * 100ms 仅够单轴冷启动. 0.4 的 init 序列 (X_ENABLE → Y_ENABLE →
     * X_SET_MODE → Y_SET_MODE) 在第二步就发到 UART3, Y 轴驱动器板冷
     * 启动典型 200-500ms, 字节被丢. 拉到 500ms 给两片都稳定时间. */
    DL_Common_delayCycles(40000000);  /* 驱动器上电稳定约 500ms @ 80MHz */
...
void SM_ReadPosition(sm_motor_t motor) {
    /* 2026-07-18: per-axis pending cache. 两轴同时 SM_ReadPosition 不会互相覆盖 */
...
void SM_AckFrame(uint8_t func) {
    /* 兼容旧 API: 收到 READ_POSITION 应答时清对应轴 pending (按轴独立) */
