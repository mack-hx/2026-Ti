 ...
 *     7. TB6612_SetPwm(A, 75);                       ← 直接设 75% 占空比 (不走档位)
 *     8. TB6612_SetCompare(A, 800);                  ← 直接设 compare=800 (PID 用)
 *
 * ============================================================================
 */
#ifndef __TB6612_H__
...
/* 读当前实际 compare 值 (TIMG12 CCP 寄存器值)
 *   - 用于主页显示实际 PWM */
uint16_t TB6612_GetCompare(tb_motor_t motor);

/* 直接设置 PWM 占空比 (不经过档位系统)
 *   pct ∈ [0, 100] → 占空比 0%~100%
 *   - 只改 PWM, 不改方向; 需要先调 TB6612_Run 设好方向
 *   - 调用后 s_pwm_comp[motor] 同步更新, GetCompare 返回新值
 *   - pct=0 等效于停转, pct=100 等效于满速 */
void TB6612_SetPwm(tb_motor_t motor, uint8_t pct);

/* 直接设置 PWM compare 原始值 (不经过百分比/档位转换)
 *   comp ∈ [0, TB_PWM_PERIOD] → 0~2000
 *   - 用于需要精确控制 compare 值的场景 (例如 PID 输出)
 *   - 只改 PWM, 不改方向
 *   - 调用后 s_pwm_comp[motor] 同步更新 */
void TB6612_SetCompare(tb_motor_t motor, uint16_t comp);
