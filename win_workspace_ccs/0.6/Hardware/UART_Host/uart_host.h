...
/* K230 摄像头命令解析结果 (主循环/UI 读取)
 *
 * K230 发送的协议 (文本行, \r\n 帧尾):
 *   "{x} {y}"    → flag=4 实时坐标 (x=黑框中心x, y=激光相对黑框y偏移)
 *   "center_point: [x, y]" → flag=1 中心点坐标
 *   "识别错误"    → 检测失败
 */
#define K230_CMD_LEN  32
typedef struct {
    bool       has_coord;     /* 本帧是否有有效坐标 */
    int32_t    coord_x;       /* 黑框中心 x */
    int32_t    coord_y;       /* y 偏移 / 中心点 y */
    bool       recognition_ok;/* 本帧识别是否成功 */
    bool       center_mode;   /* center_point 模式 vs 实时坐标模式 */
} k230_cmd_t;
extern volatile k230_cmd_t g_k230_cmd;

/* 解析 K230 文本命令 (由主循环在 UART1_RxFlag==1 时调用) */
