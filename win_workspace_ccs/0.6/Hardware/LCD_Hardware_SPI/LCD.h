...
/* ============================================================================
 * @file    LCD.h
 * @brief   1.8寸 TFT LCD 驱动头文件 (ST7735S, SPI1 Motorola 3-wire, 128×160)
 *
 *   应用层用法:
 *     - 上电: LCD_Init(BLUE);                         ← 清屏蓝色, 开背光
 *     - 画图: LCD_Fill / LCD_DrawPoint;
 *     - 字符串: LCD_ShowChar / LCD_ShowString / LCD_ShowIntNum / LCD_ShowFloatNum;
 *     - 格式化: LCD_Printf(...);
 *     - 图片: LCD_ShowPicture(x, y, length, width, pic);
 *
 *   注意: 应用层通常用 ui.c 的 row_put() 间接调用本文件 API, 不直接调.
 *
 *   屏幕方向宏 USE_HORIZONTAL: 0/1=竖屏 128×160; 2/3=横屏 160×128
 *   行布局: 一行 16 字符 (字体 LCD_8X16 = 8 px 宽), 共 10 行 (160/16)
 *
 * ============================================================================
 */
