 ...
 *   模块命名含义:
 *     - pd42_* = PD42S1 协议底层
 *     - axis_index = 驱动器地址 → 数组下标 (X=0, Y=1)
 *     - checksum8 = 8 位校验和 (包含帧头)
 *     - sm_rx_reset = 接收状态机复位
 *     - pd42_tx_byte = 发送单字节 (带 5ms 超时保护)
 *
 * ============================================================================
 ...
 *             PD42S1_UART_CallbackFor(SM_X, DL_UART_receiveData(x_bujin_INST));
 *         }
 *     }
 *
 * ============================================================================
...
/* 用户 2026-07-16 02:45 反馈: SM_Tick 卡死在 PD42S1_SendCommand 里
 *   根因: DL_UART_transmitDataBlocking 内部用 isBusy 死等 TX FIFO,
 *         在 3.3V-only 模式下 (无 12V 驱动器供电) 可能卡死 → 主循环永远阻塞
 *
 *   修复: 用 DL_UART_transmitData (非阻塞) + isTXFIFOEmpty + 超时保护.
 *   跟 0.5/Hardware/UART_Host/uart_host.c send_byte() 同样的修复模式.
 *
 *   超时 5ms / 字节: 115200 baud = 87us/字节, 5ms 足够 50+ 字节.
 *   超时后放弃这一字节, 不让主循环卡死. PD42S1 协议帧 ≤ 64 字节,
 *   一帧发送失败会让驱动器收不到, 后续节流器会按 SM_ReadPosition 自动重试. */
/* pd42_tx_byte: 发送单字节 (带 5ms 超时保护)
 *
 * 用户 2026-07-16 修复: 原 DL_UART_transmitDataBlocking 内部用 isBusy 死等,
 * 在 3.3V-only 模式下可能卡死 → 主循环永远阻塞.
 * 改用非阻塞 + 5ms 超时保护, 超时放弃该字节不卡死.
 *
 * 超时 5ms / 字节: 115200 baud ≈ 87us/字节, 5ms 足够 50+ 字节. */
static void pd42_tx_byte(UART_Regs *uart, uint8_t b) {
    uint32_t start = tick_ms;
    DL_UART_transmitData(uart, b);
    while (!DL_UART_isTXFIFOEmpty(uart)) {
        if ((uint32_t)(tick_ms - start) > 5U) {
            return;  /* 超时放弃, 不让主循环卡死 */
        }
    }
}
...
/* axis_index: 驱动器地址 → 数组下标 */
static inline uint8_t axis_index(uint8_t addr) {
...
/* checksum8: 8 位校验和
 *
 * 协议规定: CHECKSUM = sum([HEAD..DATA最后一字节]) & 0xFF, 包含帧头 0xC5 */
...
/* sm_rx_reset: 接收状态机复位 (状态机头部)
 *
 * 必须在收到合法帧尾、校验失败、长度异常等所有"退出"路径上调用,
 * 否则下一次新帧的 HEAD (0xC5) 会被当成数据字节写到 rx_buffer[1],
 * 后续收到的字节都会偏移一格, CHKSUM/TAIL 全部错位,
 * 导致 K3 读位置看起来"有时灵有时不灵" */
static inline void sm_rx_reset(uint8_t *idx) {
    *idx = 0;
}

/* PD42S1_UART_CallbackFor: UART 中断回调 (按轴)
 *
 * 协议帧格式: [C5][ADDR][FUNC][ERR][DATA...][CHECKSUM][5C]
 * 最小帧长 = 7 字节 */
...
void PD42S1_SendCommand(uint8_t addr, uint8_t func_code, uint8_t *data, uint8_t len) {
     ...
     *
     * 用户 2026-07-16: 不再用 DL_UART_transmitDataBlocking (内部 isBusy 死等
     *   在 3.3V-only 模式下可能卡死导致主循环永久阻塞). 改用非阻塞 + 5ms 超时. */
    for (uint8_t i = 0; i < tx_index; i++) {
        if (addr == 0x02U) {
            pd42_tx_byte(y_bujin_INST, tx_buf[i]);
        } else {
            pd42_tx_byte(x_bujin_INST, tx_buf[i]);
        }
    }
