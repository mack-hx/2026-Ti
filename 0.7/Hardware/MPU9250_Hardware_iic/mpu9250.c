/* ============================================================================
 * @file    mpu9250.c
 * @brief   MPU-9250 9 轴硬件 I2C 驱动 - 非阻塞状态机
 ...
 *   I2C 寄存器读写走硬件 I2C0 (PA0=SDA, PA1=SCL, 100kHz)
 *
 *   量纲:
 *     acc [g]   = raw / 16384
 *     gyro [dps]= raw / 131
...
static void dbg_putc(char c) {
    /* 用户 2026-07-16 反馈 cold-boot hang: 同 uart_host send_byte 的
     * "时间戳在跑但程序没跑" 修复, 优先 isTXFIFOEmpty + 5ms 超时,
     * 永不永久卡死 (host 开串口能恢复是因为 RX ISR 唤醒 UART 状态,
     * 我们这里主动放弃避免主循环冻在 TX 轮询) */
    DL_UART_Main_transmitData(UART_0_INST, (uint8_t)c);
    uint32_t t0 = (uint32_t)tick_ms;
    while (!DL_UART_Main_isTXFIFOEmpty(UART_0_INST)) {
        if ((uint32_t)((uint32_t)tick_ms - t0) > 5U) return;   /* 5ms 超时放弃本字节 */
    }
}
...
static volatile uint32_t s_wait_until_ms = 0;  /* 截止时刻 deadline */
...
void MPU9250_TIM_INST_IRQHandler(void) {
    if (DL_TimerG_getPendingInterrupt(MPU9250_TIM_INST) == DL_TIMER_IIDX_ZERO) {
        s_tick_flag = true;  /* 只设标志, 实际 I2C 在主循环执行, 避免与 WT9011G4K 总线冲突 */
    }
}
