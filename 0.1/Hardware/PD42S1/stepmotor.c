/**
 * @file    stepmotor.c
 * @brief   PD42S1 步进电机应用层 - 高层封装 (非阻塞)
 * @note    所有延时由底层驱动管，应用层不 sleep 也不读按键
 *
 * ============================================================================
 * 函数调用方法 (主循环只调用这些函数即可驱动电机，不接触协议细节)
 * ============================================================================
 *
 * --- 上电一次 -----------------------------------------------------------
 *     SM_Init();                  // 入队使能+模式+保存+清零 (4 步, 各隔 10ms)
 *
 * --- 主循环每帧调一次 ---------------------------------------------------
 *     SM_Tick();                  // 推进节流队列, 实际发帧
 *
 ...
 *     当前位置直接作为原点: 调用时主循环应已读过位置 (sm_pos 有效)。
 *     用户按 K5 的语义是"把当前位置设为原点", 当前位置不变 (K3 读出来还是
 *     这个数), 但 0x90/0x98 写入同样的数 → 后续 K4 多圈回零时驱动器会跑去这个坐标。
 *
...
typedef struct {
    ...
    /* 上电初始化序列: ENABLE → SET_MODE → SAVE → ZERO_POS, 共 4 步 */
    ...
    /* K3 读位置自动重试:
     *   read_pending = true  → 每 10ms 自动重发 0x2A, 直到收到合法应答或超时
     *   read_last_ms        → 上一次发 0x2A 的 tick_ms (节流)
     *   read_start_ms       → 第一次发的 tick_ms (用于 500ms 超时保护)
     * 收到合法应答 (function_code == 0x2A && error_code == 0x01)
     * 由 SM_AckFrame() 清 read_pending。 */
    bool             read_pending;
...
static sm_throttle_t s_thr = {
    SM_OP_NONE, { 0, PD42_DIR_CW, 0, 0,0,0,0 }, 0, 0,
    0, { 0, OL, 0, 0, false },
    0, 0, HS,
    0, { 0, HL, 0, 0, true },
    false, 0, 0, 0
...
static sm_op_t sm_emit(sm_op_t op) {
    switch (op) {
        case SM_OP_ENABLE:
            ...
            break;
        case SM_OP_SET_MODE:
            PD42S1_SetWorkMode(s_thr.speed.addr, PD42_MODE_SPEED_LOOP);
            break;
        ...
        case SM_OP_CLEAR_STATUS:
            ...
            break;
        case SM_OP_READ_POS:
            PD42S1_ReadPosition(s_thr.speed.addr);
            break;
...
void SM_Tick(void) {
    ...
    /* 1. 上电初始化序列: ENABLE → SET_MODE → SAVE → ZERO_POS */
    if (s_thr.init_step < 4 && s_thr.pending == SM_OP_NONE) {
        if ((int32_t)(now - s_thr.next_ms) < 0) return;
        switch (s_thr.init_step) {
            case 0: s_thr.pending = SM_OP_ENABLE;       s_thr.init_step = 1; break;
            case 1: s_thr.pending = SM_OP_SET_MODE;     s_thr.init_step = 2; break;
            case 2: s_thr.pending = SM_OP_SAVE_PARAMS;  s_thr.init_step = 3; break;
            case 3: s_thr.pending = SM_OP_ZERO_POS;     s_thr.init_step = 4; break;
        }
    }
...
void SM_Init(void) {
    /* 上电后给驱动器一点时间 (一次性阻塞, 仅上电时) */
    DL_Common_delayCycles(8000000);  /* 约 100ms @ 80MHz */

    s_thr.speed.addr = SM_X;
    s_thr.init_step = 0;            /* 由 SM_Tick 推进一步骤 */
    s_thr.pending    = SM_OP_NONE;
    s_thr.next_ms    = tick_ms;     /* 立即可发第一帧 */
...
void SM_ReadPosition(sm_motor_t motor) {
    /* 0x2A 读位置 - 自动重试模式:
     *   应用层只需在 K3 按下沿调一次本函数, 之后不用再调。
     *   节流器 (SM_Tick) 会每 10ms 自动发一帧 0x2A,
     *   直到:
     *     a) ISR 解析到合法应答 → 由 main.c (UI_Render) 调 SM_AckFrame 清 read_pending
     *     b) 超过 SM_READ_TIMEOUT_MS (500ms) → 自动放弃
     *   这种"按下一次反复询问直到收到数据"的方式能彻底解决 RX 状态机失同步
     *   导致单次发送被丢的问题 (无论丢在哪一帧, 下一帧会重新对齐)。 */
    s_thr.read_addr       = (uint8_t)motor;
    s_thr.read_pending    = true;
    s_thr.read_last_ms    = tick_ms - SM_FRAME_GAP_MS;  /* 立即可发第一帧 */
    s_thr.read_start_ms   = tick_ms;
    /* 不修改 s_thr.pending, 让 SM_Tick 在节流窗口到时自然挂上 SM_OP_READ_POS。 */
}

/**
 * @brief   通知应用层: 已收到一个有效应答帧。
 * @param   func  应答帧的功能码 (例如 PD42_FCT_READ_POSITION = 0x2A)
 * @note    由 main.c / UI_Render 在读完 PD42S1_GetFrame() 之后调用。
 *          当 func 与当前 read_pending 匹配, 清掉 read_pending, 停止重试。
 *          不匹配的应答 (例如驱动器同时回了别的帧) 不影响重试。 */
void SM_AckFrame(uint8_t func) {
    if (!s_thr.read_pending) return;
...
void SM_zeroset(sm_motor_t motor, sm_origin_t origin,
    ...
    /* 入参 → 节流器字段
     *   motor       → s_thr.speed.addr + s_thr.zs.addr  (驱动从机地址)
     *   origin      → s_thr.zs.origin       (OL→0x90, OR→0x98)
     *   timeout_ms  → s_thr.zs.timeout_ms   (回零超时, 0x95 写入值)
     *   auto_home_on→ s_thr.zs.auto_home    (true=打开上电自动回零, 0x97)
     *   sm_pos      → s_thr.zs.origin_pulses(当前位置 = 原点坐标, 0x90/0x98 写入值)
     * 序列化进 s_thr.zs, 由 SM_Tick 按 10ms 间隔推进。
     * speed.addr 同步: 作为后续指令 (SM_OP_SET_ORIGIN 等) 的 addr。 */
    s_thr.speed.addr    = (uint8_t)motor;          /* 共用: 同时是 ZERO_POS 的 addr */
    s_thr.zs.addr         = (uint8_t)motor;          /* 0x90/0x98 等指令专用 */
    s_thr.zs.origin       = origin;                  /* LEFT/RIGHT 限位选择 */
    s_thr.zs.origin_pulses= sm_pos;                  /* 把当前位置作为原点坐标 (0x90/0x98 写入值) */
    s_thr.zs.timeout_ms   = timeout_ms;              /* 回零超时 (ms) */
    s_thr.zs.auto_home    = auto_home_on;            /* 上电自动回零标志 */
    s_thr.zs_step         = 1;                       /* 启动 4 步序列 (不包含 0xF8 清零) */
}
