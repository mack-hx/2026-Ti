 ...
 *     [3] MPU9250Page   9 轴 IMU (加速度 + 陀螺 + 磁力计 + 倾斜补偿磁航向)
 *
 *   UI_Render 每帧:
 ...
 * ============================================================================ */
uint8_t       g_page = 0;
const uint8_t PAGE_COUNT = 4;   /* 与 pages[] 长度同步: Empty / Motor / HuiduTB / MPU9250 */
 ...
 *   ROW 1  y= 16   ax:+0.12 ay:+.03         加速度 X/Y [g]
 *   ROW 2  y= 32   az:+9.81 gx:+.50         加速度 Z [g] + 陀螺 X [dps]
 *   ROW 3  y= 48   gy:-.12 gz:-.20          陀螺 Y/Z [dps]
 *   ROW 4  y= 64   mx:+132  my:-45          磁力计 X/Y [uT]
 *   ROW 5  y= 80   mz:+67   yaw:123°        磁力计 Z [uT] + 倾斜补偿磁航向 [deg]
 *   ROW 6  y= 96   pit:-12  rol:+5          俯仰/翻转 [deg]
 *   ROW 7  y=112   tmp:+27.3°C              内部温度 [°C]
 *   ROW 8  y=128   dev:71 mag:48 cal        MPU WHO_AM_I (0x71) + mag (0x48) + cal OK
 *   ROW 9  y=144   页脚
 *
 *   按键语义 (本页专用, 用户 2026-07-13 决定):
 *     K1 / K2 / K3 / K4  全 no-op — 纯展示, 不做任何控制 (后续陀螺零漂校准再说)
 *     K5                 main.c 切页
 *
 *   显示策略:
 *     - 数据未就绪 (g_mpu9250.ok=false) → ROW 1..8 显示 "------"
 *     - 磁力计未读到 (who_mag != 0x48)  → ROW 4..6 mag/yaw 显示 "---", ROW 8 显示 "NO"
 *
 * ============================================================================ */
static void MPU9250Page_Render(void) {
    ...
    if (!d->ok) {
        ...
        for (uint8_t r = 1; r <= 8; r++) {
            row_put(r, "  no dev", GRAY, BLACK);
        }
        return;
    }

    /* ROW 1: ax ay
     *   "ax:%+.2f ay:%+.2f"  → ax 前缀+1+1+4=6, ay 前缀+1+1+4=6, 空格 2 = 14 字符
     *   例: "ax:-1.20 ay:+0.34"
     */
    snprintf(line, sizeof(line), "ax:%+5.2f ay:%+5.2f", d->ax, d->ay);
    row_put(1, line, WHITE, BLACK);

    /* ROW 2: az gx */
    snprintf(line, sizeof(line), "az:%+5.2f gx:%+5.2f", d->az, d->gx);
    row_put(2, line, WHITE, BLACK);

    /* ROW 3: gy gz */
    snprintf(line, sizeof(line), "gy:%+5.2f gz:%+5.2f", d->gy, d->gz);
    row_put(3, line, WHITE, BLACK);

    /* ROW 4..6: 磁力计 + 姿态, 仅在 AK8963 WHO_AM_I 读通时画真值 */
    if (d->who_mag == AK8963_WHO_AM_I_VAL) {
        /* ROW 4: mx my (uT, 整数)  "mx:%+4d my:%+4d " = 16 字符 */
        snprintf(line, sizeof(line), "mx:%+4d my:%+4d ",
                 (int)d->mx, (int)d->my);
        row_put(4, line, WHITE, BLACK);

        /* ROW 5: mz yaw (yaw 是 0..359 整数度)
         *   "mz:%+4d yaw:%3d " = 16 字符 (无 °, LCD 字库不保证有 °)
         */
        snprintf(line, sizeof(line), "mz:%+4d yaw:%3d ",
                 (int)d->mz, (int)d->yaw);
        row_put(5, line, WHITE, BLACK);

        /* ROW 6: pitch roll (有符号整数度)  "pit:%+4d rol:%+4d" = 16 字符 */
        snprintf(line, sizeof(line), "pit:%+4d rol:%+4d",
                 (int)d->pitch, (int)d->roll);
        row_put(6, line, WHITE, BLACK);
    } else {
        /* 磁力计失败 — 显示 n.c. (no connection) */
        row_put(4, "mag:------      ", GRAY, BLACK);
        row_put(5, "yaw:--- pitch:  ", GRAY, BLACK);
        row_put(6, "rol:---         ", GRAY, BLACK);
    }

    /* ROW 7: 内部温度 [C]  "tmp:%+5.1fC     " = 16 字符 */
    snprintf(line, sizeof(line), "tmp:%+5.1fC     ", d->temp_c);
    row_put(7, line, CYAN, BLACK);

    /* ROW 8: WHO_AM_I + 校准状态
     *   "dev:71 mag:48 cal"  →  15 字符 (dev:XX mag:YY cal)
     *   MPU9250 没读到 → dev:0
     *   MPU6500 假片 → dev:70 mag:0 NO
     */
    if (d->who_mpu == MPU9250_WHO_AM_I_VAL) {
        if (d->who_mag == AK8963_WHO_AM_I_VAL) {
            snprintf(line, sizeof(line), "dev:71 mag:48 OK");
        } else {
            snprintf(line, sizeof(line), "dev:71 mag:0 NO");
        }
    } else if (d->who_mpu == 0x70U) {
        snprintf(line, sizeof(line), "dev:70 mag:0 NO");
    } else {
        snprintf(line, sizeof(line), "dev:%02X mag:0 NO", d->who_mpu);
    }
...
static void MPU9250Page_OnKey(void) {
    /* K1~K4 全 no-op (用户 2026-07-13 决定: 纯数据页, 不做控制)
     *   后续如果需要"陀螺零漂校准", 加:
     *     K3 down → 开始静态 5 秒零漂校准 (采集静止状态平均 gx/gy/gz)
     *     K4 down → 重置 yaw 当前方向为 0 (相对航向)
     */
    /* K5 由 main.c 处理切页 */
}
...
const page_t pages[] = {
    ...
    { "MPU9250", MPU9250Page_Render, MPU9250Page_OnKey, NULL },                     /* [3] 9 轴 IMU (I2C0) */
};
